...
confirm_plan() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    warn "DRY-RUN: nothing destructive will run."
  fi
  printf '\n'
  pick_yesno "Proceed with this plan?" "Y"
}
...
chroot_step3_main() {
  ...
  if [[ -n "${DE_PKGS:-}" ]]; then
    ...
    pac_retry pacman -S --noconfirm --needed ${DE_PKGS}
  fi
  # shellcheck disable=SC2086
  if [[ -n "${EXTRA_PKGS:-}" ]]; then
    pac_retry pacman -S --noconfirm --needed ${EXTRA_PKGS}
  fi
  if [[ "${GPU}" == "nvidia" ]]; then
...
step3() {
  ...
  if [[ "$DESKTOP" == "gnome" ]]; then
    ...
    must "package plasma" chroot_has plasma || true
